...
A [bounded context](https://martinfowler.com/bliki/BoundedContext.html) is a sub-system in a software architecture aligned to a part of your domain. A bounded contexts can be seen as a unit of language consistency, a boundary for a model expressed in a consistent language tailored for a specific purpose. As a starting heuristic, it is an understandability boundary: things you need to understand together want to be together.
